...
1. **Never break layer boundaries** — see `.claude/guides/architecture.md`. All v2.0 additions slot into existing layers.
...
* **Every new genre profile must have a Skill file** under `.claude/skills/genres/<primary>/<subgenre>.md`.
...
1. **Write the Skill file** at `.claude/skills/genres/<primary>/<subgenre>.md` following the template in `.claude/guides/genre-development.md`. Include: overview, instrumentation, tempo/meter, harmonic vocab, melodic vocab, rhythmic vocab, structure templates, evaluation criteria, antipatterns, abstract references, constraints.
...
* **2026-05-16**: Phase 3 complete: synth instruments expanded (4→24 GM synths), vocal schema (VocalSpec + vocal_lead role), outcome-based learning in Layer 7 (update_from_outcome). All 4022 tests pass.
...
* **2026-04-28**: Stochastic generator, generator registry, musical error messages, queryable provenance, CLAUDE.md tiered guides.
...
| [Genre Development](.claude/guides/genre-development.md) *(v2.0)* | Adding a new genre Skill + profile |
| [Drum Development](.claude/guides/drum-development.md) *(v2.0)* | Adding drum patterns, working with DrumPart |
...
**Adding a genre profile?** → `.claude/skills/genres/` + `src/yao/constants/genre_profiles.py` + `specs/templates/genres/` + `tests/scenarios/`
